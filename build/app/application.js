'use strict';
angular.module('com.att.controllers.config', []).constant('config', {
  'audioPath': 'media/audio/',
  'dataPath': 'rest/api/data/'
});
'use strict';
var videoData;
var svmodule = angular.module('smartvideo', [
    'ngRoute',
    'ngSanitize',
    'com.att.controllers.config'
  ]);
svmodule.controller('SVMainController', [
  '$rootScope',
  '$scope',
  '$http',
  '$q',
      ...
      if (type != 'ecare') {
        $http.get(config.dataPath + id + '/script.json').success(function (res) {
          ...
          if (type == 'adobeanimate') {
            ...
            $('#cc').attr('src', 'assets/data/hs_caption.vtt');
            $('#subtit').attr('src', 'assets/data/hs_subtitle.vtt');
            $('#chptr').attr('src', 'assets/data/hs_chapter.vtt');
            $('#videoViewport').append($('#canvasvideo'));
            ...
            if ($rootScope.datatype != undefined) {
              if ($rootScope.datatype == 'html') {
                ...
                $('#audioSource').attr('src', videoData.media);
                if (videoData.media != undefined) {
                  if (videoData.media.indexOf('.m4a') == -1) {
                ...
                $('#cc').attr('src', 'assets/data/os_caption.vtt');
                $('#subtit').attr('src', 'assets/data/os_subtitle.vtt');
                $('#chptr').attr('src', 'assets/data/os_chapter.vtt');
                ...
                $('#videoSource').attr('src', videoData.media);
                if (videoData.media != undefined) {
                  if (videoData.media.indexOf('.m4a') == -1) {
