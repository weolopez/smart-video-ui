'use strict';
svmodule.controllerProvider.register('animationController', [
  '$scope',
  '$http',
  '$rootScope',
  function ($scope, $http, $rootScope) {
    ...
    function assignData(data) {
      ...
      $('.productImg').attr('src', 'app/template/orderstatus/images/' + data.content.productImg);
    ...
    function runVideo() {
      if ($rootScope.datatype == 'html') {
          ...
          pop.code({
            ...
            onStart: function (options) {
              if (options.ind == 0) {
                ...
                $('#webpagediv' + options.ind).css('visibility', 'visible');
              }
              if (options.opt != undefined) {
                ...
                switch (options.opt) {
                case 'zoomin':
                  $('#webpagediv' + options.ind + ' .effect').addClass('animated zoomIn');
                  break;
                case 'flipin':
                  $('#webpagediv' + options.ind + ' .effect').addClass('animated flipInY');
                  break;
                case 'rubber':
                  $('#webpagediv' + options.ind + ' .effect').addClass('animated rubberBand');
                  break;
                }
                if (options.classname != undefined) {
                  options.classname.forEach(function (item) {
                    if (item.count != undefined) {
                      $('#webpagediv' + options.ind).find(item.element).css('animation-iteration-count', item.count);
                    }
                    $('#webpagediv' + options.ind).find(item.element).addClass(item.classname);
                  });
                }
              } else {
            ...
            onEnd: function (options) {
              if (options.opt != undefined) {
                switch (options.opt) {
                case 'zoomin':
                  $('#webpagediv' + options.ind + ' .effect').removeClass('animated zoomIn').width();
                  break;
                case 'flipin':
                  $('#webpagediv' + options.ind + ' .effect').removeClass('animated flipInY').width();
                  break;
                case 'rubber':
                  $('#webpagediv' + options.ind + ' .effect').removeClass('animated rubberBand').width();
                  break;
                }
                if (options.classname != undefined) {
                  options.classname.forEach(function (item) {
                    if (item.count != undefined) {
                      $('#webpagediv' + options.ind).find(item.element).css('animation-iteration-count', '0');
                    }
                    $('#webpagediv' + options.ind).find(item.element).removeClass(item.classname).width();
                  });
                }
        ...
        $('#videoViewport').insertBefore($('.vjs-tech'));
