'use strict';
svmodule.controllerProvider.register('homesolutionController', [
  '$scope',
  '$http',
  '$rootScope',
  function ($scope, $http, $rootScope) {
    ...
    function assignData(data) {
      var audio = $('#ourvideo');
      $('#audioSource').attr('src', 'app/template/homesolution/audio/' + data.data.audio);
      $rootScope.$broadcast('changeplayer');
      audio[0].pause();
      audio[0].load();
      $('.product').html(data.data.product);
      $('.memory').html(data.data.memory);
      $('.scheduledTime').html(data.data.scheduled);
      $('.scheduledDate').html(data.data.date);
